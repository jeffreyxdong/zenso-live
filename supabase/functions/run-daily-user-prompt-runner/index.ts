...
serve(async (req) => {
  ...
  try {
    console.log('Starting daily user-generated prompt runner...');

    // Get all active user-generated prompts that have brand names
    ...
    if (promptsError) {
      console.error('Error fetching user-generated prompts:', promptsError);
      throw promptsError;
    }

    console.log(`Found ${prompts?.length || 0} active user-generated prompts to process`);

    for (const prompt of prompts || []) {
      try {
        console.log(`Processing user-generated prompt ${prompt.id} for brand: ${prompt.brand_name}`);
        ...
        if (responses.length > 0) {
          ...
          try {
            ...
            // Save daily score entry (NOT on the prompt itself)
            ...
            if (!existingScore) {
              ...
              console.log(`Saved daily scores for user-generated prompt ${prompt.id} - Visibility: ${visibilityScore}, Sentiment: ${sentimentScore}`);
            } else {
              console.log(`Daily score already exists for user-generated prompt ${prompt.id} on ${today}`);
            }
        ...
        console.error(`Error processing user-generated prompt ${prompt.id}:`, error);
      }
    }

    console.log('Daily user-generated prompt runner completed successfully');
    ...
    console.error('Error in daily user-generated prompt runner:', error);
