...
serve(async (req) => {
  ...
  try {
    ...
    if (!competitors || competitors.length === 0) {
      ...
      return new Response(
        JSON.stringify({ message: 'No competitors to score', scoresCalculated: 0 }),
    ...
    // Get recent brand prompt responses to analyze competitor mentions
    ...
    if (!responses || responses.length === 0) {
      ...
      return new Response(
        JSON.stringify({ message: 'No responses to analyze', scoresCalculated: 0 }),
        { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }

    console.log(`Analyzing ${responses.length} responses for competitor mentions`);
    ...
    for (const competitor of competitors) {
      ...
      const competitorMentions = responses.filter(response => {
        ...
        const wordBoundaryMatch = new RegExp(`\\b${competitorLower.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`, 'i').test(response.response_text);
        const partialMatch = competitorLower.split(' ').some((word: string) => word.length > 2 && contentLower.includes(word));
      ...
      for (const mention of competitorMentions.slice(0, 10)) { // Limit to 10 most recent mentions
