...
const Dashboard = () => {
  ...
  const loadBrandAnalytics = async (forceRescore = false) => {
    ...
    try {
      ...
      // No score available yet - check if store is new (created within last 5 minutes)
      ...
      const isNewStore = storeData?.created_at && 
        (new Date().getTime() - new Date(storeData.created_at).getTime()) < 5 * 60 * 1000;

      setBrandVisibility(null);
      setBrandVisibilityUpdatedAt(null);
      
      // If store is new, keep loading state active (scores are being generated)
                  ...
                  <div className="text-center">
                    {isLoadingAnalytics ? (
                      <div className="flex flex-col items-center justify-center space-y-3">
                        ...
                        <div className="text-center">
                          <p className="text-sm font-medium">Calculating AI visibility score...</p>
              ...
              <CardContent className="flex-1 overflow-y-auto">
                {isLoadingRecommendations ? (
                  <div className="flex flex-col items-center justify-center py-12 space-y-3">
                    <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-primary"></div>
                    <div className="text-center">
                      <p className="text-sm font-medium">Generating AI optimization suggestions...</p>
                      <p className="text-xs text-muted-foreground mt-1">This may take a moment</p>
                    </div>
                  </div>
                ) : brandRecommendations.length > 0 ? (
